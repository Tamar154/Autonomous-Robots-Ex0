...
# Create DataFrame
output_data = {
    'GPS time': [timestamp] * len(sv_position),
    'SatPRN (ID)': sv_position.index,
    'Sat.X': sv_position['x_k'],
    'Sat.Y': sv_position['y_k'],
    'Sat.Z': sv_position['z_k'],
    'Pseudo-Range': pr,
    'CN0': one_epoch['Cn0DbHz'],
    'Pos.X': x[0],
    'Pos.Y': x[1],
    'Pos.Z': x[2],
    'Lat': navpy.ecef2lla(x)[0],
    'Lon': navpy.ecef2lla(x)[1],
    'Alt': navpy.ecef2lla(x)[2]
}
...
for i, (lat, lon, alt) in enumerate(zip(lla_df['Latitude'], lla_df['Longitude'], lla_df['Altitude'])):
    kml.newpoint(name=f"Point {i+1}", coords=[(lon, lat, alt)])
